...
# x, y, z, yaw, time
ROUTE = [
    (0.2, 1.9, FLIGHT_HEIGHT, 0.0, 0),
    (0, 2.2, FLIGHT_HEIGHT, 0.0, 0),
    (-1.6, 2.5, FLIGHT_HEIGHT, 0.0, 0),
    (-1.6, 2.2, FLIGHT_HEIGHT, 0.0, 0),
...
def register_station_observation(
    ...
    with stations_lock:
        ...
        if (
            not track["dust_confirmed"]
            and track["dust_votes"] >= MIN_DUST_VOTES
            and dust_ratio >= DUST_VOTE_RATIO
        ):
            dusty_stations_count = sum(
                1
                for existing_track in station_tracks
                if existing_track.get("dust_confirmed", False)
            )

            if dusty_stations_count < MAX_DUSTY_STATIONS:
